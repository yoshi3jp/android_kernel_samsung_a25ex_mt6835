...
def_alloc:
	 ...
	 */
	if (s->name) {
		u64 type = get_kdp_kmem_cache_type(s->name);

		if (type) {
			u64 sc, va_page = (u64)__va(page_to_phys(page));
...
#ifdef CONFIG_RKP
	if (is_rkp_ro_buffer((u64)va_page)) {
		for (sc = 0; sc < (1 << order); sc++) {
			uh_call(UH_APP_KDP, PGD_RWX, (u64)va_page, 0, 0, 0);
...
#ifdef CONFIG_SLUB_DEBUG
#ifdef CONFIG_KDP
	if (is_kdp_kmem_cache(page->slab_cache))
		goto next;
#endif
