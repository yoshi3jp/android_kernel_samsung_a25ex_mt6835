...
#define IMGSENSOR_READ_OTP_CAL(dual_device_id, sensor_id, addr, data, size)                     imgsensor_read_otp_cal(dual_device_id, sensor_id, addr, data, size)
#define IMGSENSOR_GET_OTP_BANK_NUM(sensor_id)                                                   imgsensor_get_otp_bank_num(sensor_id)
...
static inline unsigned int IMGSENSOR_READ_OTP_CAL(unsigned int dual_device_id, unsigned int sensor_id,unsigned int addr, unsigned char *data, unsigned int size) {
	return -1;
}
static inline int IMGSENSOR_GET_OTP_BANK_NUM(sensor_id) {
	return -1;
}
