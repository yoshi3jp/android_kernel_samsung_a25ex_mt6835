...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[] = {
	...
	{ 2592, 1944,    0,    0, 2592, 1944, 2592, 1944,
	     8,    6, 2416, 1660,    0,    0, 2416, 1660 }, /*custom2*/
	{ 2592, 1944,    0,    0, 2592, 1944, 2592, 1944,
	     8,    6, 2416, 1660,    0,    0, 2416, 1660 }, /*custom3*/
	{ 2592, 1944,    0,    0, 2592, 1944, 2592, 1944,
	     8,    6, 2416, 1660,    0,    0, 2416, 1660 }, /*custom4*/
	{ 2592, 1944,    0,    0, 2592, 1944, 2592, 1944,
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	itemp = read_cmos_sensor_8(0x3221);
	itemp &= ~0x66;

	switch (image_mirror) {

	case IMAGE_NORMAL:
		write_cmos_sensor_8(0x3221, itemp);
		break;

	case IMAGE_V_MIRROR:
		write_cmos_sensor_8(0x3221, itemp | 0x06);
		break;

	case IMAGE_H_MIRROR:
		write_cmos_sensor_8(0x3221, itemp | 0x60);
		break;

	case IMAGE_HV_MIRROR:
		write_cmos_sensor_8(0x3221, itemp | 0x66);
		break;
	}
}
...
int sc501cs_otp_init(unsigned int otp_bank)
{
	kal_uint8 status = 1;
	kal_int32 cnt = 0;
	...
	status = 1;
	cnt = 0;
	while (status == 0x01 && cnt < 5) {
		status = (kal_uint8)read_cmos_sensor_8(0x4420);
		status &= 0x01;
		mDELAY(10);
		LOG_INF("read completed check is still ongoing (cnt:%d)\n", cnt++);
	}
...
int sc501cs_read_otp_cal(unsigned int addr, unsigned char *data, unsigned int size)
{
	...
	if (read_cmos_sensor_8(SC501CS_OTP_BANK1_START_ADDR) == VALID_BANK_MARK) {
		otp_bank = SC501CS_OTP_BANK1_MARK;
		read_addr = SC501CS_OTP_BANK1_START_ADDR;
	} else {
		otp_bank = SC501CS_OTP_BANK2_MARK;
		read_addr = SC501CS_OTP_BANK2_START_ADDR;
	}
	LOG_INF("check OTP bank:0x%x, size: %d\n", otp_bank, size);
	if (sc501cs_otp_init(otp_bank) < 0) {
		LOG_ERR("init fail\n");
	...
	/* bank2 checking */
	if (otp_bank == SC501CS_OTP_BANK2_MARK && read_cmos_sensor_8(SC501CS_OTP_BANK2_START_ADDR) != VALID_BANK_MARK) {
		LOG_ERR("bank2 invalid\n");
		sc501cs_otp_off_setting();
		return -1;
	}
	...
	for (i = 0; i < size; i++) {
		*(data + i) = (kal_uint8)read_cmos_sensor_8(read_addr);
		LOG_DBG("read data read_addr: %#06x, data: %#04x", read_addr, *(data + i));
		read_addr++;

		if (read_addr == SC501CS_SECOND_OTP_ADDR)
			sc501cs_otp_init(SC501CS_OTP_BANK2_MARK);
	}
...
int sc501cs_get_otp_bank_num(void)
{
	int bank_num = -1;
