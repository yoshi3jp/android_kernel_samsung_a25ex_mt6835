...
#define ISP_VENDOR_JDM     'J' // J means JDM
#endif
...
#if IS_ENABLED(CONFIG_CAMERA_AP2AP_CAL_CONVERSION)
	case ISP_VENDOR_JDM:
		pr_info("[%s] Allow the JDM camera module", __func__);
		return true;
#endif
#ifdef CONFIG_CAMERA_AAU_V32
	case ISP_VENDOR_QC:
...
static int sysfs_kernel_read(const char *name, unsigned char *data, unsigned int *size)
{
	...
	*size = fw->size;
	pr_info("[%s] file name:%s, size:%d Bytes", __func__, name, fw->size);
...
#if IS_ENABLED(CONFIG_CAMERA_AP2AP_CAL_CONVERSION)
	address_boundary = rom_addr->rom_converted_max_cal_size;
#else
...
{
	int max_num_of_rom_info = 0, i = 0, cal_size = 0, tmp = 0;
...
#if IS_ENABLED(CONFIG_CAMERA_AP2AP_CAL_CONVERSION)
				tmp = vendor_rom_info[i].rom_addr->rom_converted_max_cal_size;
#else
				tmp = vendor_rom_info[i].rom_addr->rom_max_cal_size;
#endif
...
#if IS_ENABLED(CONFIG_CAMERA_AP2AP_CAL_CONVERSION)
				cal_size = vendor_rom_info[i].rom_addr->rom_converted_max_cal_size;
#else
