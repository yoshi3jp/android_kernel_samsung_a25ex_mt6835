		...
		unsigned int addr, unsigned char *data, unsigned int size)
{
	...
	case GC02M1B_SENSOR_ID:
		...
		break;
#endif
#if defined(GC08A3F_MIPI_RAW)
	case GC08A3F_SENSOR_ID:
		ret = gc08a3_read_otp_cal(addr, data, size);
		break;
#endif
...
int imgsensor_get_otp_bank_num(unsigned int sensor_id)
{
	int bank_num = -1;
...
#if defined(SC501CS_MIPI_RAW)
	case SC501CS_SENSOR_ID:
		bank_num = sc501cs_get_otp_bank_num();
		break;
#endif
	default:
		bank_num = 0;
...
EXPORT_SYMBOL(imgsensor_get_otp_bank_num);
