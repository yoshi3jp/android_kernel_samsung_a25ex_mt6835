	...
	struct IMGSENSOR_HW_DEVICE_COMMON *pcommon)
{
	...
	char str_pinctrl_name[LENGTH_FOR_SNPRINTF] = "none";
	...
	if (IS_ERR(pgpio->ppinctrl)) {
		PK_PR_ERR("ERROR: %s, Cannot find camera pinctrl!", __func__);
	...
	for (j = IMGSENSOR_SENSOR_IDX_MIN_NUM;
		...
		for (i = 0 ; i < GPIO_CTRL_STATE_MAX_NUM_CAM; i++) {
			...
			if (lookup_names) {
				ret = snprintf(str_pinctrl_name, sizeof(str_pinctrl_name), "cam%d_%s", j, lookup_names);
				if (ret < 0)
					PK_PR_ERR("NOITCE: %s, snprintf err, %d\n", __func__, ret);

				pgpio->ppinctrl_state_cam[j][i] = pinctrl_lookup_state(pgpio->ppinctrl, str_pinctrl_name);
			}

			if (pgpio->ppinctrl_state_cam[j][i] == NULL ||
				IS_ERR(pgpio->ppinctrl_state_cam[j][i])) {
				PK_PR_ERR("pinctrl err(%s), name:%s, pinctrl_state: %#x",
						__func__, str_pinctrl_name, pgpio->ppinctrl_state_cam[j][i]);
	...
	for (i = 0; i < GPIO_CTRL_STATE_MAX_NUM_SWITCH; i++) {
		if (gpio_pinctrl_list_switch[i].ppinctrl_lookup_names) {
			pgpio->ppinctrl_state_switch[i] =
				pinctrl_lookup_state(pgpio->ppinctrl, gpio_pinctrl_list_switch[i].ppinctrl_lookup_names);
		}

		if (pgpio->ppinctrl_state_switch[i] == NULL ||
			IS_ERR(pgpio->ppinctrl_state_switch[i])) {
			PK_PR_ERR("pinctrl err(%s), name:%s, pinctrl_state: %#x", __func__,
					gpio_pinctrl_list_switch[i].ppinctrl_lookup_names, pgpio->ppinctrl_state_switch[i]);
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	PK_INFO("[GPIO %s] sensor_idx = %d, pin = %d", gpio_state ? "low" : "high", sensor_idx, pin);
	...
	if (ppinctrl_state != NULL && !IS_ERR(ppinctrl_state))
		...
		PK_PR_ERR("pinctrl err(%s), PinIdx: %d, Val: %d, pinctrl_state: %#x",
				__func__, pin, pin_state, ppinctrl_state);
