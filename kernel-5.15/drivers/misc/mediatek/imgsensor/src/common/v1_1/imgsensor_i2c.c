...
enum IMGSENSOR_RETURN imgsensor_i2c_buffer_mode(int enable)
{
	PK_INFO("not support i2c_buf_mode\n");
	return IMGSENSOR_RETURN_SUCCESS;
}
#endif
		...
		int speed)
{
	...
	if (pinst->pi2c_client == NULL) {
		PK_INFO("NOTICE: pi2c_client is NULL!\n");
		return IMGSENSOR_RETURN_ERROR;
	}
	...
	if (i2c_ret != IMGSENSOR_I2C_MSG_SIZE_READ) {
		...
		if (__ratelimit(&ratelimit))
			PK_INFO("NOTICE: I2C[ch:%d, device id:%#x] read failed (%d)! speed(%d) (0x%x)\n",
				pinst->pi2c_client->adapter->nr, id, i2c_ret, speed, *pwrite_data);
		...
		int speed)
{
	...
	if (pinst->pi2c_client == NULL) {
		PK_INFO("NOTICE: slave_id %d pi2c_client is NULL!\n", id);
		return IMGSENSOR_RETURN_ERROR;
	}
	...
	if (i2c_ret != i) {
		...
		if (__ratelimit(&ratelimit))
			PK_INFO("NOTICE: I2C[ch:%d, device id:%#x] write failed (%d)! speed(%d) (0x%x)\n",
			pinst->pi2c_client->adapter->nr, id, i2c_ret, speed, *pwrite_data);
