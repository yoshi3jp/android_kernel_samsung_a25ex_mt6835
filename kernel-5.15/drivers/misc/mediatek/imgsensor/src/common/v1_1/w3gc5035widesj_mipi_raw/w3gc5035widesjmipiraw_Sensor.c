...
#include "w3gc5035widesjmipiraw_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc5035c_camera_sensor"
#define LOG_1 LOG_INF("GC5035MIPI, 2LANE\n")
/****************************   Modify end    *******************************************/
#define GC5035_DEBUG 1
#if GC5035_DEBUG
...
#define EEPROM_CN24C62PL_ID 0xA2
...
static kal_uint16 read_eeprom(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8*)&get_byte, 1, EEPROM_CN24C62PL_ID);
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = W3GC5035WIDESJ_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{ 2592, 1944,   0,   0, 2592, 1944, 2592, 1944, 0, 0, 2592, 1944, 0, 0, 2592, 1944 },
	{ 2592, 1944,  16, 252, 2560, 1440, 2560, 1440, 0, 0, 2560, 1440, 0, 0, 2560, 1440 },
	{ 2592, 1944,   0,   0, 2592, 1944, 2592, 1944, 0, 0, 2592, 1944, 0, 0, 2592, 1944 },
	{ 2592, 1944,   0,   0, 2592, 1944, 2592, 1944, 0, 0, 2592, 1944, 0, 0, 2592, 1944 },
	{ 2592, 1944,   0,   0, 2592, 1944, 2592, 1944, 0, 0, 2592, 1944, 0, 0, 2592, 1944 }
};
...
static struct stCAM_CAL_DATAINFO_STRUCT w3gc5035widesj_eeprom_data ={
	.sensorID= W3GC5035WIDESJ_SENSOR_ID,
	.deviceID = 0x04,
	.dataLength = 0x0778,
	.sensorVendorid = 0x1B030000,
	.vendorByte = {1,2,3,4},
	.dataBuffer = NULL,
};
static struct stCAM_CAL_CHECKSUM_STRUCT w3gc5035widesj_checksum[6] =
{
...
static kal_uint16 gc5035c_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	...
	iReadRegI2CTiming(pu_send_cmd, 1, (u8 *) &get_byte, 1,imgsensor.i2c_write_id, imgsensor_info.i2c_speed);
	return get_byte;
}
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	LOG_INF("framerate %d, min_fl:%d fl:%d\n", framerate, imgsensor.min_frame_length, frame_length);
}
...
static void set_shutter(kal_uint16 shutter)
{
	...
	if (imgsensor.autoflicker_en) {
		if (realtime_fps >= 297 && realtime_fps <= 305)
			...
			write_cmos_sensor(0x42, imgsensor.frame_length & 0xfe);
		}
	} else {
	...
	LOG_INF("Exit! cal_shutter = %d flick_en %d\n", cal_shutter,imgsensor.autoflicker_en);
}
                     ...
                     kal_bool auto_extend_en)
{
	...
	LOG_INF("shutter:%d,fl:%d rt_fps:%d \n", shutter, imgsensor.frame_length,realtime_fps);
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_init_gc5035c[] = {
...
kal_uint16 addr_data_pair_preview_gc5035c[] = {
	/* System */
	0xfe, 0x00,
	0x3e, 0x01,
	0xfc, 0x01,
...
kal_uint16 addr_data_pair_capture1_gc5035c[] = {
...
kal_uint16 addr_data_pair_capture2_gc5035c[] = {
...
kal_uint16 addr_data_pair_normal_video_gc5035c[] = {
	/* System */
	0xfe, 0x00,
	0x3e, 0x01,
	0xfc, 0x01,
...
kal_uint16 addr_data_pair_custom1_gc5035c[] = {
...
static void sensor_init(void)
{
	LOG_INF("E\n");
	#if MULTI_WRITE
        pr_info("sensor_init i2c burst mode E\n");
		gc5035c_table_write_cmos_sensor(
			addr_data_pair_init_gc5035c,
			sizeof(addr_data_pair_init_gc5035c) /
			sizeof(kal_uint16));
	#else
		/* SYSTEM */
		write_cmos_sensor(0xfc, 0x01);
...
static void preview_setting(void)
{
	LOG_INF("E\n");
	#if MULTI_WRITE
        pr_info("preview_setting i2c burst mode E\n");
		gc5035c_table_write_cmos_sensor(
			addr_data_pair_preview_gc5035c,
			sizeof(addr_data_pair_preview_gc5035c) /
			sizeof(kal_uint16));
	#else
		/* System */
		write_cmos_sensor(0xfe, 0x00);
...
static void capture_setting(kal_uint16 currefps)
{
	...
	#if MULTI_WRITE
		pr_info("capture_setting i2c burst mode E\n");
		if (currefps == 240) { /* PIP */
			gc5035c_table_write_cmos_sensor(
				addr_data_pair_capture1_gc5035c,
				sizeof(addr_data_pair_capture1_gc5035c) /
				sizeof(kal_uint16));
		} else {
			gc5035c_table_write_cmos_sensor(
				addr_data_pair_capture2_gc5035c,
				sizeof(addr_data_pair_capture2_gc5035c) /
				sizeof(kal_uint16));
		}
	#else
		/* System */
...
static void normal_video_setting(kal_uint16 currefps)
{
	LOG_INF("E! currefps: %d\n", currefps);
	#if MULTI_WRITE
		pr_info("normal_video_setting i2c burst mode E\n");
		gc5035c_table_write_cmos_sensor(
			addr_data_pair_normal_video_gc5035c,
			sizeof(addr_data_pair_normal_video_gc5035c) /
			sizeof(kal_uint16));
	#else
        /* System */
		write_cmos_sensor(0xfe, 0x00);
...
static void custom1_setting(kal_uint16 currefps)
{
	LOG_INF("E! currefps: %d\n", currefps);
	#if MULTI_WRITE
    pr_info("custom1_setting i2c burst mode E\n");
    gc5035c_table_write_cmos_sensor(
        addr_data_pair_custom1_gc5035c,
        sizeof(addr_data_pair_custom1_gc5035c) /
        sizeof(kal_uint16));
	#else
		/* System */
		write_cmos_sensor(0xfe, 0x00);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				gc5035_otp_identify();
				//+w3 camera bring up --otp porting
				imgSensorReadEepromData(&w3gc5035widesj_eeprom_data,w3gc5035widesj_checksum);
				if((w3gc5035widesj_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001)) {
					pr_err("cwd w3gc5035widesj get eeprom data failed\n");
					if(w3gc5035widesj_eeprom_data.dataBuffer != NULL) {
						kfree(w3gc5035widesj_eeprom_data.dataBuffer);
						w3gc5035widesj_eeprom_data.dataBuffer = NULL;
					}
					//*sensor_id = 0xFFFFFFFF;
					//return ERROR_SENSOR_CONNECT_FAIL;
				} else {
					pr_info("cwd w3gc5035widesj get eeprom data success\n");
				}
				//-w3 camera bring up --otp porting
				pr_info("[gc5035_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			pr_err("[gc5035_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		pr_err("w3gc5035widesj Read id fail,sensor id: 0x%x\n", *sensor_id);
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_info("[gc5035_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_err("[gc5035_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_lock(&imgsensor_drv_lock);
	imgsensor.sensor_mode = IMGSENSOR_MODE_COUSTOM1;
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_SHUTTER_FRAME_TIME:
		LOG_INF("SET_SHUTTER_FRAME_TIME ,%d, %d,%d\n",
			(UINT16)*feature_data, (UINT16)*(feature_data + 1), (UINT16)*(feature_data + 2));
...
UINT32 W3GC5035WIDESJ_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
