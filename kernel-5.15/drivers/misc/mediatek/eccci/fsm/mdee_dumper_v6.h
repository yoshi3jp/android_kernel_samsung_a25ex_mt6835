...
#ifndef __MDEE_DUMPER_V6_H__
#define __MDEE_DUMPER_V6_H__
...
#define MD_CORE_TOTAL_NUM       (5)
#define MIPS_VPE_NUM            (15)
...
union ex_main_content {
	struct ex_fatal_v6 fatalerr;
	struct ex_assert_v6 assert;
} __packed;
...
} __packed EX_BRIEF_MAININFO_T_v6;

struct ex_main_reason_v6 {
...
struct ex_overview_t {
	u32 overview_verno;
	u32 core_num;
	struct ex_main_reason_v6 main_reson[MD_CORE_TOTAL_NUM];
	struct ex_brief_maininfo_v6 ex_info;
	u32 mips_vpe_num;/* value == 7 */
	struct ex_step_v6 ex_step_logging[MIPS_VPE_NUM];
	u8 ex_reserved;
	u8 ex_ect_cpu_index;
	u16 ex_ect_thread_intex;
	u32 afound_buffer_offset;
	u32 afound_buffer_size;
	u8 usip_scq_offending_core;
	u8 sonic_offending_core;
	u8 mcu_exception_count[MIPS_VPE_NUM];
	u8 pad[67];
...
struct dump_info_fatal {
	...
	char *err_sec;
	char offender[64];
	/*must be larger than struct ex_fatal_v6 filename:
	 * pre-fix + fatal_fname
	 */
