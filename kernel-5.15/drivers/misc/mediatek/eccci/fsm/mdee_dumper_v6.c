...
static struct md_info mdee_info_v6;
...
static void mdee_info_dump_v6(struct ccci_fsm_ee *mdee)
{
	...
	ret = snprintf(mdee_info_v6.md_ee_info_buf, mdee_info_v6.buf_len, "%s", ex_info);
...
int mdee_dumper_v6_alloc(struct ccci_fsm_ee *mdee)
{
	...
	mrdump_mini_add_extra_file((unsigned long)ex_info, __pa_nodebug(ex_info),
		512, "MD_EE_INFO");
#endif
	mdee_info_v6.buf_len = 512;
	mdee_info_v6.md_ee_info_buf = ex_info;
#if IS_ENABLED(CONFIG_MTK_EMI)
