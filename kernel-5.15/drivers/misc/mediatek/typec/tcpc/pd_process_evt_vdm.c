...
{
	struct tcp_dpm_custom_vdm_data *uvdm_data =
		&pd_port->tcp_event.tcp_dpm_data.uvdm_data;

	pd_port->uvdm_wait_resp = uvdm_data->wait_resp;
	pd_port->uvdm_cable = uvdm_data->cable;
	pd_port->uvdm_cnt = uvdm_data->cnt;
	memcpy(pd_port->uvdm_data, uvdm_data->data,
	       sizeof(pd_port->uvdm_data[0]) * pd_port->uvdm_cnt);
	if ((pd_port->uvdm_data[0] & VDO_SVDM_TYPE) && pd_check_rev30(pd_port))
		pd_port->uvdm_data[0] |= VDO_SVDM_VER(SVDM_REV20);
