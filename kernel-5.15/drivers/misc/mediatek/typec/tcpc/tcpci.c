...
EXPORT_SYMBOL(tcpci_ss_factory);
...
{
	struct tcp_notify tcp_noti;
	struct tcp_ny_uvdm *uvdm_msg = &tcp_noti.uvdm_msg;
	struct pd_port *pd_port = &tcpc->pd_port;

	uvdm_msg->ack = ack;

	if (ack) {
		uvdm_msg->cable = pd_port->uvdm_cable;
		uvdm_msg->cnt = pd_port->uvdm_cnt;
		memcpy(uvdm_msg->data, pd_port->uvdm_data,
		       sizeof(uvdm_msg->data[0]) * uvdm_msg->cnt);
	}
