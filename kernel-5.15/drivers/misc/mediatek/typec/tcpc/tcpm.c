	...
	const struct tcp_dpm_event_cb_data *cb_data)
{
	...
	if (uvdm_data->cnt > PD_DATA_OBJ_SIZE)
		return TCPM_ERROR_PARAMETER;

	memcpy(&tcp_event.tcp_dpm_data.uvdm_data, uvdm_data,
	       sizeof(*uvdm_data));
	...
	if (cb_data == NULL && ret == TCP_DPM_RET_SUCCESS &&
	    uvdm_data->wait_resp) {
		mutex_lock(&pd_port->pd_lock);
		uvdm_data->cnt = pd_port->uvdm_cnt;
		memcpy(uvdm_data->data, pd_port->uvdm_data,
		       sizeof(uvdm_data->data[0]) * uvdm_data->cnt);
		mutex_unlock(&pd_port->pd_lock);
	}
