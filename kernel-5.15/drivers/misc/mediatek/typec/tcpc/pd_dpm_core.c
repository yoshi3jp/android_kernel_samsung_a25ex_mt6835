...
static int dpm_ufp_response_svids(struct pd_port *pd_port)
{
	...
	svids[0] = VDO_SVID(SAMSUNG_VID, 0); 
#else
	struct svdm_svid_data *svid_data;
...
#if IS_ENABLED(CONFIG_PDIC_NOTIFIER)
	if (pd_is_support_modal_operation(pd_port) || pd_port->is_send_svid)
#else
...
#if IS_ENABLED(CONFIG_PDIC_NOTIFIER)
		else if (pd_port->data_role == PD_ROLE_DFP
						&& vid == SAMSUNG_VID)
...
{
	pd_send_custom_vdm(pd_port, pd_port->uvdm_cable ? TCPC_TX_SOP_PRIME : TCPC_TX_SOP);
...
void pd_dpm_dfp_inform_uvdm(struct pd_port *pd_port, bool ack)
{
	...
	struct svdm_svid_data *svid_data = pd_port->uvdm_cable ?
