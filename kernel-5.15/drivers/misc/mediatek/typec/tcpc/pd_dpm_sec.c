...
void sec_dfp_accessory_detach_handler(struct pd_port *pd_port)
{
	PD_NOTI_TYPEDEF pdic_noti = {
		...
		.dest = PDIC_NOTIFY_DEV_ALL,
		.id = PDIC_NOTIFY_ID_CLEAR_INFO,
		.sub1 = 0,
		.sub2 = 0,
		.sub3 = 0,
	};
	...
	pdic_noti.sub1 = PDIC_NOTIFY_ID_DEVICE_INFO;
	pdic_notifier_notify((PD_NOTI_TYPEDEF *) &pdic_noti, NULL, 0);

	pdic_noti.sub1 = PDIC_NOTIFY_ID_SVID_INFO;
	pdic_notifier_notify((PD_NOTI_TYPEDEF *) &pdic_noti, NULL, 0);
}
	...
	struct svdm_svid_data *svid_data, bool ack)
{
	...
	pdic_noti.sub1 = pd_port->sec_vid;
	pdic_noti.sub2 = pd_port->sec_pid;
	pdic_noti.sub3 = pd_port->sec_bcd_device;
	pdic_notifier_notify((PD_NOTI_TYPEDEF *) &pdic_noti, NULL, 0);
...
bool sec_dfp_notify_discover_modes(
	struct pd_port *pd_port, struct svdm_svid_data *svid_data, bool ack)
{
	pr_info("%s ack=%d state=%d mode_cnt=%d svid=0x%04x\n", __func__, ack,
...
bool sec_dfp_notify_pe_ready(struct pd_port *pd_port,
