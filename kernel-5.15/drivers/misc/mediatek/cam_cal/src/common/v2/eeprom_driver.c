...
static int eeprom_ioctl_get_command_data(enum CAM_CAL_COMMAND command, unsigned int device_id, unsigned int sensor_id);
...
#if !IS_ENABLED(CONFIG_CAMERA_AP2AP_CAL_CONVERSION)
			    ...
			    size_t size, loff_t *offset)
{
	...
	must_log("sysfs update after using ap2ap cal conversion\n");
	if (IMGSENSOR_SYSFS_UPDATE(kbuf, pdata->sensor_info.device_id, pdata->sensor_info.sensor_id, 0, size, size) < 0)
		pr_err("sysfs update failed\n");
...
static int eeprom_ioctl_get_command_data(enum CAM_CAL_COMMAND command, unsigned int device_id, unsigned int sensor_id)
{
	...
	case CAM_CAL_COMMAND_MEMTYPE:
		...
		info = -EINVAL;
		break;
	case CAM_CAL_COMMAND_BANK_NUM:
		info = IMGSENSOR_GET_OTP_BANK_NUM(sensor_id);
		break;
...
long compat_eeprom_ioctl_control_command(void *pBuff)
{
	...
	if (copy_to_user(compat_ptr(((struct COMPAT_CAM_CAL_SENSOR_INFO *)pBuff)->info), &info, sizeof(info)))
		return -EFAULT;
