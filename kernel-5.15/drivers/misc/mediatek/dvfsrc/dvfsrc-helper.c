...
static const struct dvfsrc_debug_data mt6835_data = {
	.version = 0x6835,
	.config = &mt6983_dvfsrc_config,
	.opps_desc = dvfsrc_opp_mt6873_desc,
	.num_opp_desc = ARRAY_SIZE(dvfsrc_opp_mt6873_desc),
...
int dvfsrc_panic_notifier(struct notifier_block *this, unsigned long event, void *ptr)
{
	int err = 0;

	pr_info("call %s\n", __func__);

	if (!dvfsrc_drv || !(dvfsrc_drv->regs)) {
		pr_err("Relevant drv,io-registers are not initialized");
		err = -ENXIO;
	}

	writel(0x30, dvfsrc_drv->regs + 0x2c);
	udelay(500);
...
static struct notifier_block panic_blk = {
	.notifier_call = dvfsrc_panic_notifier,
	.priority = INT_MAX,
};
