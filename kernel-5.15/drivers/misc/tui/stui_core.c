...
lbl_rollback_touch:
	...
	case STUI_HW_IOCTL_FINISH_TUI: {
		...
		if (stui_check_mode(STUI_MODE_OFF)) {
...
int stui_open_touch(void)
{
	...
	if (stui_check_mode(STUI_MODE_TOUCH_SEC)) {
...
int stui_open_display(struct tui_hw_buffer *buffer)
{
	...
	if (stui_check_mode(STUI_MODE_DISPLAY_SEC)) {
		pr_err(TUIHW_LOG_TAG " already in TUI mode.\n");
		return -EBUSY;
	}
...
{
	pr_debug(TUIHW_LOG_TAG " %s >>\n", __func__);
	if (stui_check_mode(STUI_MODE_TOUCH_SEC) == 0) {
		pr_err(TUIHW_LOG_TAG " already free.\n");
		return;
	}
	// Clear mask first, or restoring register fails in "stui_tsp_exit"
...
{
	pr_debug(TUIHW_LOG_TAG " %s >>\n", __func__);
	if (stui_check_mode(STUI_MODE_DISPLAY_SEC) == 0) {
		pr_err(TUIHW_LOG_TAG " already free.\n");
		return;
	}
