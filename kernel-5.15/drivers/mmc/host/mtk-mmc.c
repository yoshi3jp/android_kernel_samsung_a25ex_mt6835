...
#include <linux/of_gpio.h> //dunweichao.wt,2024.4.8,add proc file for sdcard slot detect
...
static const struct mtk_mmc_compatible mt6835_compat = {
	.clk_div_bits = 12,
	.recheck_sdio_irq = false,
	.hs400_tune = false,
	.pad_tune_reg = MSDC_PAD_TUNE0,
...
static int msdc_tune_response(struct mmc_host *mmc, u32 opcode)
{
	...
	int cmd_err = 0;
	int i, j;
	dev_info(host->dev,"[%s %d]opcode=%d\n", __func__, __LINE__, opcode);
...
skip_fall:
	...
	final_maxlen = max(final_rise_delay.maxlen, final_fall_delay.maxlen);
	if (final_maxlen == final_rise_delay.maxlen) {
		dev_info(host->dev, "[%s %d]rise\n", __func__, __LINE__);
		sdr_clr_bits(host->base + MSDC_IOCON, MSDC_IOCON_RSPL);
		sdr_clr_bits(host->base + MSDC_IOCON,
			     MSDC_IOCON_DSPL | MSDC_IOCON_W_DSPL);
		final_delay = final_rise_delay.final_phase;
	} else {
		dev_info(host->dev, "[%s %d]fall\n", __func__, __LINE__);
		sdr_set_bits(host->base + MSDC_IOCON, MSDC_IOCON_RSPL);
		sdr_set_bits(host->base + MSDC_IOCON,
			     MSDC_IOCON_DSPL | MSDC_IOCON_W_DSPL);
		final_delay = final_fall_delay.final_phase;
...
void msdc_set_bad_card_and_remove(struct msdc_host *host)
{
	...
	mmc->caps |=MMC_CAP_UHS_SDR104 | MMC_CAP_UHS_DDR50 | MMC_CAP_UHS_SDR50 | MMC_CAP_UHS_SDR25;
}
...
int sdcard_reset_tuning_down(struct mmc_host *mmc)
{
	struct msdc_host *host = mmc_priv(mmc);
	char *remove_cap;
	int ret = 0;
	...
	if (mmc_card_uhs(mmc->card)) {
		dev_err(host->dev, "%s sd3_bus_mode %d\n", __func__, mmc->card->sw_caps.sd3_bus_mode);
...
tune_done:
	...
	if (bad_sd_detecter[0] >= 4 && !host->block_bad_card) {
		if (!(mmc->caps2 & MMC_CAP2_NO_SD))
			sdcard_reset_tuning_down(mmc);
	}
...
static int cd_gpio = 0;//dunweichao.wt,2024.4.8,add proc file for sdcard slot detect
				   ...
				   struct msdc_host *host)
{
	...
	of_property_read_u32(pdev->dev.of_node, "mediatek,hs400-cmd-int-delay",
			     &host->hs400_cmd_int_delay);
	cd_gpio = of_get_named_gpio(pdev->dev.of_node,"cd-gpios",0);//dunweichao.wt,2024.4.8,add proc file for sdcard slot detect
...
host_free:
	...
	if(!strcmp(mmc_hostname(mmc),"mmc1"))
		sim_card_tray_remove_proc(); //dunweichao.wt,2024.4.8,add proc file for sdcard slot detect
	return ret;
}
