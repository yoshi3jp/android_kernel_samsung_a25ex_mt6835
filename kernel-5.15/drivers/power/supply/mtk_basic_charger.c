...
//+ liwei19.wt modify 20240521 disable battery temperature protect
	...
	} else if (info->disable_quick_charge) {
		/* set current for 15w pd*/
	...
	if (en == false) {
		...
		for (i = 0; i < MAX_ALG_NO; i++) {
			...
			if (alg->alg_id == PE5_ID) {
				chg_alg_set_prop(alg, ALG_CHARGING_STATE, WT_STOP_CHARGING);
			}
			chg_alg_stop_algo(alg);
		}
		charger_dev_enable(info->chg1_dev, false);
		...
		if(info->pd_type == MTK_PD_CONNECT_PE_READY_SNK_APDO) {
			alg = get_chg_alg_by_name("pe5");
			if (alg != NULL) {
				chg_alg_set_prop(alg, ALG_CHARGING_STATE, WT_RECOVERY_CHARGING);
