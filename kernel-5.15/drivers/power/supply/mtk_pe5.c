...
#define WT_PE50_CV_HIGH1       4160
#define WT_PE50_CV_HIGH2       4280
#define WT_PE50_ITA_CC_HIGH1   2430
#define WT_PE50_ITA_CC_HIGH2   1900
#define PE50_COMMON_VTA        9000	/* mV */
#define PE50_COMMON_ITA        1100  /* mA */
#define FIX_PDO_COMMON_INPUT_CURRENT      1100  /* mA */
#define AC_CHARGER_CURRENT     2050  /* mA */

#define WT_ITA_OFFSET    50
...
static int pe50_init_tbat_level(struct pe50_algo_info *info);
...
static inline int pe50_get_ita_lmt(struct pe50_algo_info *info)
{
...
#ifdef ENABLE_WT_JEITA
	int ret = 0;
	int vbat = WT_PE50_CV_HIGH2;
	...
	if ((vbat >= WT_PE50_CV_HIGH2) && (data->cv_input_current_limit != WT_PE50_ITA_CC_HIGH2)) {
		if (data->step_count_timer2++ > STEP_TIMER) {
			data->cv_input_current_limit = WT_PE50_ITA_CC_HIGH2;
			data->step_count_timer2 = 0;
		}
	} else if ((vbat >= WT_PE50_CV_HIGH1) && (data->cv_input_current_limit == -1)) {
		if (data->step_count_timer1++ > STEP_TIMER) {
			data->cv_input_current_limit = WT_PE50_ITA_CC_HIGH1;
			data->step_count_timer1 = 0;
		}
	} else {
		data->step_count_timer1 = 0;
		data->step_count_timer2 = 0;
	}
	PE50_INFO("vbat=%d, cv_input_current_limit=%d, data->ita_lmt=%d\n", vbat, data->cv_input_current_limit, data->ita_lmt);
#endif
...
#ifdef ENABLE_WT_JEITA
	if (data->cv_input_current_limit >= 0)
		ita = min(ita, (u32)data->cv_input_current_limit);
#endif
	...
	if (data->tried_dual_dvchg) {
		...
		if (desc->tbat_curlmt[data->tbat_level] >= 0) {
...
static inline void pe50_init_algo_data(struct pe50_algo_info *info)
{
	...
	if (!data->is_ita_limit_stop) {
		//data->tbat_level = PE50_THERMAL_NORMAL;
		data->tbat_level = pe50_init_tbat_level(info);
	}
	...
	data->run_once = true;
	data->state = PE50_ALGO_INIT;
	data->is_ita_limit_stop = false;
	data->wt_charging_state = WT_START_CHARGING;
#ifdef ENABLE_WT_JEITA
	data->cv_input_current_limit = -1;
	data->step_count_timer1 = 0;
	data->step_count_timer2 = 0;
#endif
	data->is_wakeup_timer = false;
	mutex_lock(&data->notify_lock);
	data->notify = 0;
	mutex_unlock(&data->notify_lock);
...
static int pe50_algo_init_with_ta_cv(struct pe50_algo_info *info)
{
	...
	if (ret < 0) {
		...
		//+P240722-07242, wangshewen.wt 202201029,Green Union PD30W charger recognition PPS.
		//sinfo.hardreset_ta = true;
		//goto err;
		//-P240722-07242, wangshewen.wt 202201029,Green Union PD30W charger recognition PPS.
	}
...
static int pe50_algo_cc_cv_with_ta_cv(struct pe50_algo_info *info)
{
	...
	u32 wt_ita_gap_per_vstep = WT_ITA_OFFSET;
	struct pe50_algo_desc *desc = info->desc;
	struct pe50_stop_info sinfo = {
		.reset_ta = true,
		.hardreset_ta = false,
	...
	ita_gap_per_vstep = min(ita_gap_per_vstep, wt_ita_gap_per_vstep);
	ret = pe50_get_adc(info, PE50_ADCCHAN_VBAT, &vbat);
	if (ret < 0) {
		PE50_ERR("get vbat fail(%d)\n", ret);
		goto out;
			       ...
			       struct pe50_stop_info *sinfo)
{
	...
	if (!data->is_dvchg_en[PE50_DVCHG_MASTER])
		return true;
	data->ita_setting = ita_level[PE50_RCABLE_NORMAL];
				     ...
				     struct pe50_thermal_data *tdata)
{
	if (tdata->temp >= tdata->temp_level_def[PE50_THERMAL_VERY_HOT]) {
		...
		if (strncmp(tdata->name, "tbat", 4) == 0) {
			*tdata->temp_level = PE50_THERMAL_VERY_HOT;
		}
		return false;
	}
	if (tdata->temp <= tdata->temp_level_def[PE50_THERMAL_VERY_COLD]) {
		...
		if (strncmp(tdata->name, "tbat", 4) == 0) {
			*tdata->temp_level = PE50_THERMAL_VERY_COLD;
		}
		return false;
	}
	switch (*tdata->temp_level) {
	case PE50_THERMAL_VERY_COLD:  //-5
		if (tdata->temp >= (tdata->temp_level_def[PE50_THERMAL_VERY_COLD] +
				    tdata->recovery_area)) {
			*tdata->temp_level = PE50_THERMAL_COLD;
		} else {
			PE50_ERR("tbat between %d and %d,not allow charging yet!!\n",
				tdata->temp_level_def[PE50_THERMAL_VERY_COLD],
				tdata->temp_level_def[PE50_THERMAL_VERY_COLD] + tdata->recovery_area);
		}
		break;
	case PE50_THERMAL_COLD:  // 4
		...
		if (tdata->temp >= tdata->temp_level_def[PE50_THERMAL_WARM]) {
			if (strncmp(tdata->name, "tbat", 4) == 0) {
				...
				*tdata->temp_level = PE50_THERMAL_WARM;
				...
				if (tdata->curlmt[PE50_THERMAL_WARM] > 0)
					*tdata->temp_level = PE50_THERMAL_WARM;
			}
		}
		else if (tdata->temp <=
	...
	case PE50_THERMAL_WARM:  //30
		...
		break;
	case PE50_THERMAL_VERY_WARM: //40
		if (tdata->temp <=
		    (tdata->temp_level_def[PE50_THERMAL_VERY_WARM] -
		     tdata->recovery_area))
			*tdata->temp_level = PE50_THERMAL_WARM;
		...
		break;
	case PE50_THERMAL_HOT:  //45
		if (tdata->temp <= (tdata->temp_level_def[PE50_THERMAL_HOT] -
				    tdata->recovery_area))
			*tdata->temp_level = PE50_THERMAL_VERY_WARM;
		break;
	case PE50_THERMAL_VERY_HOT:  //55
		if (tdata->temp <= (tdata->temp_level_def[PE50_THERMAL_VERY_HOT] -
				    tdata->recovery_area)) {
			*tdata->temp_level = PE50_THERMAL_HOT;
		} else {
			PE50_ERR("tbat between %d and %d,not allow charging yet!!\n",
				tdata->temp_level_def[PE50_THERMAL_VERY_HOT] - tdata->recovery_area,
				tdata->temp_level_def[PE50_THERMAL_VERY_HOT]);
		}
		break;
	default:
...
static int pe50_init_tbat_level(struct pe50_algo_info *info)
{
	int ret, tbat;
	int i = 0;
	enum pe50_thermal_level temp_level = PE50_THERMAL_NORMAL;
	struct pe50_algo_data *data = info->data;
	struct pe50_algo_desc *desc = info->desc;
	struct pe50_thermal_data tdata = {
		.name = "tbat",
	...
	if (ret < 0) {
		PE50_ERR("get tbat fail(%d)\n", ret);
		return PE50_THERMAL_NORMAL;
	}

	tdata.temp = tbat;

	for (i = PE50_THERMAL_VERY_HOT; i >= PE50_THERMAL_VERY_COLD; i--) {
	...
	if ((temp_level < PE50_THERMAL_VERY_COLD) || (temp_level > PE50_THERMAL_VERY_HOT)) {
		temp_level = PE50_THERMAL_NORMAL;
	}

	PE50_INFO("%s(%d,%d)\n", tdata.name, tdata.temp, temp_level);
	return temp_level;
}
				       ...
				       struct pe50_stop_info *sinfo)
{
	...
	data->run_once = false;
	data->is_ita_limit_stop = false;
	data->wt_charging_state = WT_NOT_CHARGING;
	data->is_wakeup_timer = false;
	return pe50_stop(info, sinfo);
}
...
static int pe50_algo_threadfn(void *param)
{
	...
	while (!kthread_should_stop()) {
		...
		case PE50_ALGO_STOP:
			...
			if ((data->wt_charging_state == WT_START_CHARGING) && data->is_ita_limit_stop) {
				pe50_check_tbat_level(info, NULL);
			}
			idvchg_lmt = pe50_get_idvchg_lmt(info);
		...
		if (data->state != PE50_ALGO_STOP) {
			...
			if ((data->wt_charging_state == WT_START_CHARGING) && data->is_ita_limit_stop) {
				data->is_wakeup_timer = true;
			}

			if (data->is_wakeup_timer) {
				PE50_ERR("wt_wakeup_timer\n");
				polling_interval = desc->polling_interval;
				sec = polling_interval / 1000;
				ms = polling_interval % 1000;
				ktime = ktime_set(sec, MS_TO_NS(ms));
			      ...
			      struct chg_alg_notify *notify)
{
	...
	mutex_lock(&data->notify_lock);
	if ((data->state == PE50_ALGO_STOP) && ((data->wt_charging_state != WT_RECOVERY_CHARGING)
		&& (data->wt_charging_state != WT_RECOVERY_QUICK_CHARGE))) {
		...
		goto out;
	} else if (data->state != PE50_ALGO_STOP) {
		if (notify->evt == EVT_ALGO_RECOVERY) {
			goto out;
		}
	}
	...
	case EVT_VBUSOVP_ALARM:
		data->notify |= BIT(notify->evt);
		break;
	case EVT_ALGO_RECOVERY:
		if (data->wt_charging_state != WT_NOT_CHARGING)
			break;
	default:
		ret = -EINVAL;
		goto out;
				  ...
				  struct chg_limit_setting *setting)
{
	...
	PE50_INFO("%s: ic=%d, cv=%d, input_current_limit=%d\n",
		__func__, ic, cv, data->input_current_limit);
	mutex_lock(&data->ext_lock);
		...
		enum chg_alg_props s, int value)
{
	...
	case ALG_REF_VBAT:
		data->ref_vbat = value;
		break;
	case ALG_CHARGING_STATE:
		if ((data->wt_charging_state == WT_STOP_CHARGING) && (value == WT_RECOVERY_CHARGING)) {
			data->wt_charging_state = WT_RECOVERY_CHARGING;
		} else if ((data->wt_charging_state != WT_NOT_CHARGING) && (value != WT_RECOVERY_CHARGING)) {
			data->wt_charging_state = value;
		}
		pr_notice("%s: wt_charging_state=%d\n", __func__, data->wt_charging_state);
		break;
	case ALG_DISABLE_AFC:
		if (value > 0) {
			data->disable_quick_charge = true;
			data->wt_charging_state = WT_DISABLE_QUICK_CHARGE;
		} else {
			data->disable_quick_charge = false;
			data->wt_charging_state = WT_RECOVERY_QUICK_CHARGE;
		}
		pr_notice("%s: disable_quick_charge=%d\n", __func__, data->disable_quick_charge);
		break;
	default:
		break;
	}
