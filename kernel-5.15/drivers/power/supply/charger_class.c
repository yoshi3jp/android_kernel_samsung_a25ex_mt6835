...
int charger_dev_send_afc_data(struct charger_device *chg_dev,
					     u32 val)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->send_afc_data)
		return chg_dev->ops->send_afc_data(chg_dev, val);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_send_afc_data);

int charger_dev_afc_set_voltage(struct charger_device *chg_dev,
					     int val)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_afc_voltage)
		return chg_dev->ops->set_afc_voltage(chg_dev, val);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_afc_set_voltage);

int charger_dev_afc_disable_set(struct charger_device *chg_dev,
					     int val)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->afc_disable_set)
		return chg_dev->ops->afc_disable_set(chg_dev, val);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_afc_disable_set);

int charger_dev_enable_hiccup(struct charger_device *chg_dev,
					     bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_enable_hiccup)
		return chg_dev->ops->set_enable_hiccup(chg_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_enable_hiccup);

int charger_dev_set_cc_hi_state(struct charger_device *chg_dev,
					     int val)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_cc_hi_state)
		return chg_dev->ops->set_cc_hi_state(chg_dev, val);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_cc_hi_state);

int charger_dev_set_water_state(struct charger_device *chg_dev,
					     bool val)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->set_water_state)
		return chg_dev->ops->set_water_state(chg_dev, val);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_water_state);
...
int charger_dev_get_vbus_level(struct charger_device *chg_dev, int *vbus)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->get_vbus_level)
		return chg_dev->ops->get_vbus_level(chg_dev, vbus);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_vbus_level);
...
int charger_dev_set_eoc_timer(struct charger_device *charger_dev,
				unsigned int mins)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
	    charger_dev->ops->set_eoc_timer)
		return charger_dev->ops->set_eoc_timer(charger_dev, mins);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_eoc_timer);

int charger_dev_enable_eoc(struct charger_device *charger_dev,
				bool en)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
		charger_dev->ops->enable_eoc)
		return charger_dev->ops->enable_eoc(charger_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_enable_eoc);

int charger_dev_enable_ship_mode(struct charger_device *charger_dev, bool batdet_dis_dly)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
	    charger_dev->ops->enable_ship_mode)
		return charger_dev->ops->enable_ship_mode(charger_dev, batdet_dis_dly);
...
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
	    charger_dev->ops->get_vsys_adc)
		return charger_dev->ops->get_vsys_adc(charger_dev, vsys);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_vsys);

int charger_dev_enable_aicc(struct charger_device *charger_dev, bool en)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
	    charger_dev->ops->enable_aicc)
		return charger_dev->ops->enable_aicc(charger_dev, en);
...
int charger_dev_en_wdt(struct charger_device *charger_dev, bool en)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
					   charger_dev->ops->en_wdt)
		return charger_dev->ops->en_wdt(charger_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_en_wdt);

int charger_dev_en_ilim(struct charger_device *charger_dev, bool en)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
	    charger_dev->ops->en_ilim)
		return charger_dev->ops->en_ilim(charger_dev, en);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_en_ilim);

int charger_dev_set_vsysmin(struct charger_device *charger_dev,
				unsigned int mV)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
	    charger_dev->ops->set_vsysmin)
		return charger_dev->ops->set_vsysmin(charger_dev, mV);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_set_vsysmin);

int charger_dev_set_iinlmtsel(struct charger_device *chg_dev, bool en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
...
int charger_dev_get_health(struct charger_device *charger_dev, int *health)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
	    charger_dev->ops->get_health)
		return charger_dev->ops->get_health(charger_dev, health);

	return -EOPNOTSUPP;
}
EXPORT_SYMBOL(charger_dev_get_health);

int charger_dev_get_charge_type(struct charger_device *charger_dev, int *type)
{
	if (charger_dev != NULL && charger_dev->ops != NULL &&
	    charger_dev->ops->get_charge_type)
		return charger_dev->ops->get_charge_type(charger_dev, type);
