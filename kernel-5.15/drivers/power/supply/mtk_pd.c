			...
			int *newidx)
{
	...
	if ((cap->nr == 0) ||
		...
		pr_info("[%s] do not execute pdc_send_pd_noti(), typec_attach_state : %d\n",
			__func__, tcpm_inquire_typec_attach_state(pd->tcpc_dev));
		return -1;
	}
	...
	*newcur = cap->ma[*newidx];
	...
	pr_info("[%s] pd_role=%s, typec_attach_state : %d\n",
		__func__, pd_role ? "SOURCE" : "SINK", tcpm_inquire_typec_attach_state(pd->tcpc_dev));
