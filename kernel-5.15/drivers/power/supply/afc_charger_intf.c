...
int afc_plugout_reset(struct mtk_charger *pinfo)
{
	...
	//S98901AA1,liwei19.wt,modify,2024/08/06, Restore the enumeration of afc after unplugging the charger
	pinfo->afc.is_enable = true;
...
EXPORT_SYMBOL(afc_get_is_connect);
...
//bug533258,zhaosidong.wt,ADD,20200217,toggle between 9V/1.65A and 5V/2A
	...
	if(abs(vchr - SET_9V) < 1000000) {
		...
		*aicr = pinfo->data.afc_common_charger_input_curr;
		*ichg = pinfo->data.afc_common_charger_curr;
	}
...
The AICR will be configured right siut value in CC step */
	...
	if (ret == 0) {
		g_afc_work_status = 1;
		if (!pinfo->disable_quick_charge) {
			ret = afc_set_mivr(pinfo, SET_9V - 1000000);
		} else {
			ret = afc_set_mivr(pinfo, pinfo->data.afc_min_charger_voltage);
		}
	} else {
		g_afc_work_status = 0;
		goto out;
...
int afc_start_algorithm(struct mtk_charger *pinfo)
{
	...
	vbat = get_battery_voltage(pinfo);
	if ((abs(pre_vbus - SET_9V) < 1000000) &&
		get_uisoc(pinfo) > 85 && ichg > 0 &&
				    ...
				    const char *buf, size_t count)
{
	...
	if (!IS_ERR_OR_NULL(psy)) {
		ret = power_supply_set_property(psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &psy_val);
		if (ret < 0) {
