...
#include "sec_mtk_charger.h"
/*
*   sec_mtk_charger.c file for kernel 5.15
*/
...
static void mtk_charger_set_charger_timer(bool enable);
...
static int mtk_charger_get_vsys_voltage(struct mtk_charger_data *charger)
{
	...
	pr_info("[DEBUG] %s: vsys_voltage val : %d\n", __func__, vsys_voltage);
...
{
	pr_info("[DEBUG] %s: called charger otg control : %s\n", __func__,
			enable ? "ON" : "OFF");
		...
		struct mtk_charger_data *charger, int float_voltage)
{
	...
	/* need to update "info->data.battery_cv" */
	value.intval = float_voltage * 1000;
	psy_do_property("mtk-master-charger", set,
...
static void mtk_charger_set_charger_timer(bool enable)
{
	...
	/*
	* enable:0 => mtk_charger_timer_force_stop()
	* enable:1 => mtk_charger_timer_force_start()
	*/
	value.intval = enable;
	psy_do_property("mtk-master-charger", set,
			POWER_SUPPLY_PROP_CHARGE_CONTROL_START_THRESHOLD, value);
}
...
static void mtk_charger_ob_mode(struct mtk_charger_data *charger, bool enable)
{
	...
	if (enable) {
		...
		pr_info("[DEBUG]%s: set buck on\n", __func__);
		charger_dev_enable_powerpath(charger->chg_dev, true);
		charger_dev_set_vsysmin(charger->chg_dev, 4000);
		...
		/* Set Input current limit to max (3225mA) */
...
static void mtk_charger_ib_mode(struct mtk_charger_data *charger, bool enable)
{
	...
	if (enable) {
		...
		pr_info("[DEBUG]%s: set buck on\n", __func__);
		charger_dev_enable_powerpath(charger->chg_dev, true);
		charger_dev_set_vsysmin(charger->chg_dev, 3600);
		if (charger->pdata->chgenb_en == 0) {
			/* SET GPIO_CHGENB = "L" */
			gpio_direction_output(charger->pdata->gpio_chgenb, 0);
		} else {
...
{
	struct mtk_charger_data *charger = data;
...
{
	struct mtk_charger_data *charger =
		container_of(work, struct mtk_charger_data, pogo_init_work.work);
	int ret = 0;
...
{
	struct mtk_charger_data *charger =
		container_of(work, struct mtk_charger_data, pogo_detect_work.work);
	...
	if (gpio_get_value(charger->pdata->gpio_pogo_int)) {
		vbus = mtk_charger_get_vbus_voltage(charger);
...
static bool mtk_charger_chg_init(struct mtk_charger_data *charger)
{
	...
	if (charger->f_mode != OB_MODE) {
		...
		charger_dev_set_eoc_timer(charger->chg_dev, 30);
		/* Enable HW EOC */
		charger_dev_enable_termination(charger->chg_dev, true);
...
static void mtk_charger_aicl_work(struct work_struct *work)
{
	struct mtk_charger_data *charger =
		container_of(work, struct mtk_charger_data, aicl_work.work);
	union power_supply_propval value = {0, };
	...
	if (is_wired_type(charger->cable_type)) {
		...
		if (charge_type == POWER_SUPPLY_CHARGE_TYPE_TRICKLE) {
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		if (!charger->is_charging) {
			...
			mtk_charger_check_aicl(charger);
		}
		break;
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		if (val->intval) {
			pr_info("[DEBUG] %s: Set OB Mode (byp keystring)\n", __func__);
			/* Set MIVR for vbus 4.2V */
			charger_dev_set_mivr(charger->chg_dev, 4200000); /* uV */
			charger->f_mode = OB_MODE;
			mtk_charger_ob_mode(charger, true);
			mtk_charger_set_charger_timer(false);
		} else {
			pr_info("[DEBUG] %s: SET IB mode (byp keystring)\n",
				__func__);
			charger->f_mode = IB_MODE;
			mtk_charger_ib_mode(charger, true);
			mtk_charger_set_charger_timer(true);
			/* Set MIVR for vbus 5V */
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		/* by AT CMD */
		if (val->intval) {
			pr_info("[DEBUG] %s: Set OB Mode (CMD)\n", __func__);
			/* Set MIVR for vbus 4.2V */
			charger_dev_set_mivr(charger->chg_dev, 4200000); /* uV */
			charger->f_mode = OB_MODE;
			mtk_charger_ob_mode(charger, true);
			...
			pr_info("[DEBUG] %s: Set IB Mode (CMD)\n", __func__);
			charger->f_mode = IB_MODE;
			mtk_charger_ib_mode(charger, true);
			mtk_charger_set_charger_timer(true);
			/* Set MIVR for vbus 5V */
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CURRENT_MEASURE:
			/* by keystring */
			if (val->intval) {
				pr_info("[DEBUG] %s: Set OB Mode (keystring)\n",
					__func__);
				/* Set MIVR for vbus 4.2V */
				charger_dev_set_mivr(charger->chg_dev, 4200000); /* uV */
				charger->f_mode = OB_MODE;
				...
				pr_info("[DEBUG] %s: Set IB Mode (keystring)\n",
					__func__);
				charger->f_mode = IB_MODE;
				mtk_charger_ib_mode(charger, true);
				mtk_charger_set_charger_timer(true);
				/* Set MIVR for vbus 4.4V */
				charger_dev_set_mivr(charger->chg_dev,
					charger->pdata->vbus_min_charger_voltage); /* uV */
			}
#if IS_ENABLED(CONFIG_USB_FACTORY_MODE)
		...
		case POWER_SUPPLY_EXT_PROP_IB_MODE:
			pr_info("[DEBUG] %s: Set IB Mode (sysfs): %d\n", __func__,
								val->intval);
			charger->f_mode = IB_MODE;
		...
		case POWER_SUPPLY_EXT_PROP_OB_MODE_CABLE_REMOVED:
			{
				...
				pr_info("[DEBUG] %s: VBUS: %d\n", __func__, vbus);
				if (vbus < 4000) {
					pr_info("%s: No Cable, power-off\n",
						__func__);
					/* BATFET tuns off with 18s delay */
					charger_dev_enable_ship_mode(charger->chg_dev, true);
					kernel_power_off();
				}
			}
			break;
		case POWER_SUPPLY_EXT_PROP_SHIPMODE_TEST:
			pr_info("[DEBUG]  %s: manual ship mode set as %s\n", __func__,
						val->intval ?
						"enable" : "disable");
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		charger->cable_type = val->intval;
		charger->slow_charging = false;
		pr_err("[DEBUG] %s charger->cable_type(%d)\n", __func__, charger->cable_type);
		if (charger->psy_bc12)
...
{
	struct device_node *np = of_find_node_by_name(NULL, "sec-mtk-charger");
	int ret = 0;

	if (!np) {
		pr_err("%s np NULL(sec-mtk-charger)\n", __func__);
	} else {
		...
		if (ret) {
			pr_info("%s: vbus_min_charger_voltage is Empty\n",
				__func__);
			pdata->vbus_min_charger_voltage = 4400000;
		}
#if defined(CONFIG_SEC_FACTORY)
		pdata->vbus_min_charger_voltage = 4200000;
#endif
...
#if defined(CONFIG_SEC_FACTORY)
		pdata->vbus_normal_mivr_voltage = 4200000;
#endif
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	pr_info("%s:[BATT] SEC MTK Charger driver probe\n", __func__);
	...
	if (ret < 0) {
		pr_err("%s : failed to request chgenb gpio\n", __func__);
		goto err_parse_gpio;
	}
	...
	if (charger->pdata->charger_name == NULL)
		charger->pdata->charger_name = "sec-mtk-charger";
	...
	/* need to get the cable type from MTK when init */
	psy_do_property("primary_chg", get,
		POWER_SUPPLY_PROP_ONLINE, value);
	if (value.intval) {
		value.intval = SEC_BATTERY_CABLE_TYPE_FROM_MTK;
		psy_do_property("primary_chg", get,
	...
	INIT_DELAYED_WORK(&charger->aicl_work, mtk_charger_aicl_work);
#if IS_ENABLED(CONFIG_USE_POGO)
	...
	pr_info("%s:[BATT] SEC MTK Charger driver loaded OK. cable type(%d)\n", __func__, value.intval);
...
static int mtk_charger_suspend(struct device *dev)
{
	...
	struct mtk_charger_data *charger = i2c_get_clientdata(i2c);
