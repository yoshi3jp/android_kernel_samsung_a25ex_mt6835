...
static ssize_t proximity_tmd4914_prox_led_test_show(char *buf)
{
	int ret = 0;
	char *buffer = NULL;
	int buffer_length = 0;
...
static ssize_t proximity_tmd4914_prox_trim_show(char *buf, int type)
{
...
static ssize_t proximity_tmd4914_prox_cal_store(const char *buf, size_t size, int type)
{
	int ret = 0;
...
static ssize_t proximity_tmd4914_debug_info_show(char *buf)
{
	int ret = 0;
	char *buffer = NULL;
	int buffer_length = 0;
...
struct proximity_factory_chipset_funcs proximity_tmd4914_ops = {
	.prox_cal_store = proximity_tmd4914_prox_cal_store,
	.prox_trim_show = proximity_tmd4914_prox_trim_show,
	.debug_info_show = proximity_tmd4914_debug_info_show,
	.prox_led_test_show = proximity_tmd4914_prox_led_test_show,
};

struct proximity_factory_chipset_funcs *get_proximity_tmd4914_chipset_func(char *name)
{
	if (strcmp(name, "TMD4914") != 0)
		return NULL;

	return &proximity_tmd4914_ops;
}
