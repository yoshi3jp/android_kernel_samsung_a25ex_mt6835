...
static void enter_error_mode(struct isg5320a_data *data, enum grip_error_state err_state)
{
	GRIP_INFO("%s - %d enter\n", __func__, data->err_state);
...
static void check_irq_error(struct isg5320a_data *data, u8 irq_state, bool is_irq_func, bool is_enable_func)
{
	u8 state;
	if (data->is_irq_active && data->check_abnormal_working == false) {
		state = irq_state & (1 << ISG5320A_PROX_STATE);
		...
		if (is_irq_func) {
			...
			GRIP_INFO("%s prev %x state %x\n", __func__, data->prev_state, state);
			data->state_miss_matching_count++;
			data->prev_state = state;
		}

		if (data->state_miss_matching_count >= 3) {
			GRIP_ERR("%s - enter_error_mode with IRQ\n",  __func__);
			enter_error_mode(data, FAIL_IRQ_MISS_MATCH);
		} else
			GRIP_INFO("%s - %d\n", __func__, data->state_miss_matching_count);
	}
}
...
static irqreturn_t isg5320a_irq_thread(int irq, void *ptr)
{
	...
	check_irq_error(data, intr_msg, true, false);
...
static void isg5320a_initialize(struct isg5320a_data *data)
{
	...
	if (data->setup_reg_exist) {
		for (i = 0; i < data->reg_size ; i++) {
			...
			while (retry--) {
				ret = isg5320a_i2c_write_one(data, data->setup_reg[index],
					data->setup_reg[index + 1]);
				if (ret >= 0) {
					ret  = isg5320a_i2c_read(data, data->setup_reg[index], &val, 1);
...
static ssize_t isg5320a_register_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct isg5320a_data *data = dev_get_drvdata(dev);
	char *p = buf;
	...
	for (idx = 0; idx < data->reg_size; idx++) {
		isg5320a_i2c_read(data, data->setup_reg[idx * 2], &val, 1);
		i += snprintf(p + i, PAGE_SIZE - i, "(0x%02x)=0x%02x\n", data->setup_reg[idx * 2], val);
	}

	return i;
}

static ssize_t isg5320a_register_write_store(struct device *dev,
			struct device_attribute *attr, const char *buf, size_t count)
{
	struct isg5320a_data *data = dev_get_drvdata(dev);
	unsigned int reg_address = 0, val = 0;

	if (sscanf(buf, "%4x,%4x", &reg_address, &val) != 2) {
		GRIP_ERR("sscanf err\n");
		return -EINVAL;
	}

	isg5320a_i2c_write_one(data, (unsigned char)reg_address, (unsigned char)val);
...
static DEVICE_ATTR(register_read_all, S_IRUGO, isg5320a_register_show, NULL);
static DEVICE_ATTR(reg_update, S_IRUGO, isg5320a_reg_update_show, NULL);
static DEVICE_ATTR(register_write, 0220, NULL, isg5320a_register_write_store);
...
static void debug_work_func(struct work_struct *work)
{
	...
	if (data->enable == ON) {
		isg5320a_i2c_read(data, ISG5320A_IRQSTS_REG, &irq_state, 1);
		GRIP_INFO("debug_work : irq_state %x\n", irq_state);
		check_irq_error(data, irq_state, false, false);
	}

	schedule_delayed_work(&data->debug_work, msecs_to_jiffies(5000));
}
...
static int isg5320a_parse_dt(struct isg5320a_data *data, struct device *dev)
{
	...
	if (data->ic_num == MAIN_GRIP) {
		data->gpio_int = of_get_named_gpio(node, "isg5320a,irq-gpio", 0);
		if (data->gpio_int < 0) {
	...
	if (data->ic_num == SUB_GRIP) {
		data->gpio_int = of_get_named_gpio(node, "isg5320a_sub,irq-gpio", 0);
		if (data->gpio_int < 0) {
			GRIP_ERR("get gpio_int error\n");
			return -ENODEV;
		}
...
void isg5320a_remove(struct i2c_client *client)
#else
...
static int isg5320a_suspend(struct device *dev)
{
	...
	if (data->enable && data->is_irq_active)
		disable_irq(data->client->irq);
