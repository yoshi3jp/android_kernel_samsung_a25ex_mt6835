...
#define PANEL_DISPLAY_MODE_POST_SEQ ("panel_display_mode_post_seq")
#endif
...
#define PANEL_FORCE_UPDATE ("panel_force_update")
#define PANEL_SET_FREQ_STEP ("panel_set_freq_step")
...
struct ddi_properties {
	...
	bool mdnie_enable_in_init_seq;
	bool no_set_pcd_bypass;
};
...
struct ddi_ops {
	...
	int (*firmware_update)(struct panel_device *panel, void *data);
	int (*firmware_info_print)(struct panel_device *panel, void *data);
#endif
};
...
struct panel_device_attr {
	...
	u32 flags;
	bool node_created[MAX_PANEL_DEV];
};
