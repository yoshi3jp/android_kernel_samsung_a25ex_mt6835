...
static int mcd_drm_parse_dt(void *_ctx, struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "MTK,vdo_per_frame_lp_enable", &ctx->spec.vdo_per_frame_lp_enable);
	dev_info(ctx->dev, "%s: vdo_per_frame_lp_enable: %d\n", __func__, ctx->spec.vdo_per_frame_lp_enable);
...
__visible_for_testing int mcd_drm_trigger_recovery(void *_ctx)
{
	...
	return mtk_drm_uevent_trigger(LCD_ON);	/* todo: (void *_ctx, int state) */
}
...
static int mcd_panel_prepare(struct drm_panel *panel)
{
	...
	ret = call_mcd_panel_func(ctx->mcd_panel_dev, reset_lp11);
	if (ret < 0) {
		dev_err(ctx->dev, "%s failed to reset_lp11 %d\n", __func__, ret);
		return 0;  /* return ret; */
	}
