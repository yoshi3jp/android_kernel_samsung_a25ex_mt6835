...
#define HX9036_DRIVER_VER "3cd39a" //Change-Id
...
#endif
static bool pause_mode_flag[HX9036_CH_NUM] = {false};
...
char sales_code_from_cmdline[SALE_CODE_STR_LENGTH + 1];
EXPORT_SYMBOL(sales_code_from_cmdline);
...
//64个可选的接近阈值设置
static uint16_t hx9036_threshold_near_num[] = {//size:64
...
static struct hx9036_near_far_threshold hx9036_ch_thres1_default_hx9036[HX9036_CH_NUM] = {
    ...
    {.thr_near = 80, .thr_far = 0},
    {.thr_near = 32767, .thr_far = 0},
    {.thr_near = 128, .thr_far = 0},
    {.thr_near = 32767, .thr_far = 0},
    {.thr_near = 96, .thr_far = 0},
    {.thr_near = 32767, .thr_far = 0},
    {.thr_near = 88, .thr_far = 0},
};
...
static void hx9036_sample_work_func(void);
...
static void hx9036_manual_offset_calibration(void)//手动校准所有通道
{
    ...
    if((pause_mode_flag[0] == true) || (pause_mode_flag[1] == true) || (pause_mode_flag[2] == true) || (pause_mode_flag[3] == true) ||
    (pause_mode_flag[4] == true) || (pause_mode_flag[5] == true) || (pause_mode_flag[6] == true) || (pause_mode_flag[7] == true)){
        PRINT_INF("chs enter the pause mode , will not cali or close.\n");
    }
    else{
...
static ssize_t hx9036_class_sar_cali(struct class *class,
                                  ...
                                  const char *buf, size_t count)
{
    ...
    if (en) {
        PRINT_INF("class_stk_sar_cali , reset all phase\n");
        mutex_lock(&hx9036_cali_mutex);
        buf1[0] = 0xFF;
        ret = hx9036_write(PAGE0_57_OFFSET_CALI_CTRL1, buf1, 1);
        if(0 != ret) {
            PRINT_ERR("hx9036_write failed\n");
        }
        PRINT_INF("chs will calibrate in next convert cycle (ODR=%dms)\n", HX9036_ODR_MS);
        ...
        PRINT_ERR("class_stk_sar_cali, input : %d, nothing work\n", en);
    }
    return count;
}
...
static void hx9036_sample(void)
{
    ...
    ret = hx9036_read(PAGE0_0B_INT_STATE_RD7, rx_buf, 1);
    PRINT_INF("PAGE0_0B_INT_STATE_RD7 = 0x%02X\n", rx_buf[0]);
    ret = hx9036_read(PAGE0_9F_CAP_INI_RD, rx_buf, 1);
    PRINT_INF("PAGE0_9F_CAP_INI_RD = 0x%02X\n", rx_buf[0]);
}
...
static int __init sales_code_setup(char *str)
{
    ...
    PRINT_INF("01sales_code_from_cmdline:%s\n", sales_code_from_cmdline);
    return 1;
 }
...
//-P86801AA1-1797, ludandan.wt,ADD, 2023/05/20, add sar power reduction control switch
        if (PROXACTIVE == touch_state) {
            if (ii == 3|| ii == 1) {
                if ((hx9036_ch_near_state[3] == PROXACTIVE ||  hx9036_ch_near_state[1] == PROXACTIVE)&& hx9036_pdata.grip_status == SAR_STATE_FAR) {
                    input_report_rel(hx9036_pdata.chs_info[1].hx9036_input_dev, REL_MISC, SAR_STATE_NEAR);
                    input_report_rel(hx9036_pdata.chs_info[1].hx9036_input_dev, REL_X, 2);
                    input_sync(hx9036_pdata.chs_info[1].hx9036_input_dev);
                    hx9036_pdata.chs_info[ii].state = PROXACTIVE;
                    hx9036_pdata.grip_status = SAR_STATE_NEAR;
                    PRINT_DBG("%s report PROXACTIVE\n", hx9036_pdata.chs_info[ii].name);
                } else {
                    PRINT_DBG("%s already PROXACTIVE, nothing report2!\n", hx9036_pdata.chs_info[ii].name);
                }
                continue;
            }
            if (hx9036_pdata.chs_info[ii].state == PROXACTIVE) {
            ...
            if (ii == 3|| ii == 1) {
                if ((hx9036_ch_near_state[3] == IDLE &&  hx9036_ch_near_state[1] == IDLE && hx9036_pdata.grip_status == SAR_STATE_NEAR)) {
                    input_report_rel(hx9036_pdata.chs_info[1].hx9036_input_dev, REL_MISC, SAR_STATE_FAR);
                    input_report_rel(hx9036_pdata.chs_info[1].hx9036_input_dev, REL_X, 2);
                    input_sync(hx9036_pdata.chs_info[1].hx9036_input_dev);
                    hx9036_pdata.chs_info[ii].state = IDLE;
                    hx9036_pdata.grip_status = SAR_STATE_FAR;
                    PRINT_DBG("%s report released\n", hx9036_pdata.chs_info[ii].name);
                } else {
                    PRINT_DBG("%s already released, nothing report!\n", hx9036_pdata.chs_info[ii].name);
                }
...
static void hx9036_enable_int_prox(uint8_t ch_id)
{
    int ret = -1;
    uint8_t buf_a2[1] = {0};
    uint8_t buf_a5[1] = {0};
    ...
    if(ch_id >= HX9036_CH_NUM) {
        PRINT_ERR("channel index over range!!!\n");
        return;
    }

    ret = hx9036_read(PAGE0_A2_INTERRUPT_CFG2, buf_a2, 1);
    ret |= hx9036_read(PAGE0_A5_INTERRUPT_CFG5, buf_a5, 1);
    ret |= hx9036_read(PAGE0_99_DSP_CONFIG_CTRL6, buf_99, 1);
    buf_a2[0] |= (1 << ch_id);
    buf_a5[0] |= (1 << ch_id);
    buf_99[0] |= (1 << ch_id);
    ret |= hx9036_write(PAGE0_A2_INTERRUPT_CFG2, buf_a2, 1);
    ret |= hx9036_write(PAGE0_A5_INTERRUPT_CFG5, buf_a5, 1);
    ret |= hx9036_write(PAGE0_99_DSP_CONFIG_CTRL6, buf_99, 1);
...
static void hx9036_disable_int_prox(uint8_t ch_id)
{
    int ret = -1;
    uint8_t buf_a2[1] = {0};
    uint8_t buf_a5[1] = {0};
...
static int hx9036_ch_en(uint8_t ch_id, uint8_t en)
{
    ...
    if(ch_enable_status > 0) {
        ...
        //hx9036_sample_work_func();
    } else {
        if(1 == en) {
            if(pause_mode_flag[ch_id]){
                PRINT_INF("ch%d enter the pause mode , will not cali or close.\n", ch_id);
                return 0;
            }
...
static int hx9036_ch_en(uint8_t ch_id, uint8_t en)
{
    ...
    if(1 == en) {
        hx9036_enable_int_prox(ch_id);
        PRINT_INF("enable int!\n");
        ch_enable_status |= (1 << ch_id);
        ...
        en = 0;
        hx9036_disable_int_prox(ch_id);
        PRINT_INF("disable int!\n");
        if(pause_mode_flag[ch_id]){
            PRINT_INF("ch%d enter the pause mode , will not cali or close.\n",ch_id);
            return 0;
        }
...
static ssize_t hx9036_offset_csn_show(struct class *class, struct class_attribute *attr, char *buf)
{
    ...
    data_offset_csn[0] = data_offset_dac[0];
    data_offset_csn[1] = data_offset_dac[1];
    data_offset_csn[2] = data_offset_dac[2];
    data_offset_csn[3] = data_offset_dac[3];
    data_offset_csn[4] = data_offset_dac[4];
    data_offset_csn[5] = data_offset_dac[5];
    data_offset_csn[6] = data_offset_dac[6];
    data_offset_csn[7] = data_offset_dac[7];
                            ...
                            const char *buf, size_t count)
{
    ...
    for (index = 0; index < HX9036_CH_NUM; index++) {
        if (!strcmp(temp_input_dev->name, hx9036_channels[index].name)) {
            if (1 == index) {
                if (0 != hx9036_ch_en(0, val)) {
                    PRINT_ERR("hx9036_ch%d_en failed\n", 0);
                    mutex_unlock(&hx9036_ch_en_mutex);
                    return -1;
                }
                if (0 != hx9036_ch_en(2, val)) {
                    PRINT_ERR("hx9036_ch%d_en failed\n", 2);
                    mutex_unlock(&hx9036_ch_en_mutex);
                    return -1;
                }
                if (0 != hx9036_ch_en(3, val)) {
                    PRINT_ERR("hx9036_ch%d_en failed\n", 3);
                    mutex_unlock(&hx9036_ch_en_mutex);
                    return -1;
                }
...
//            wake_lock_timeout(&hx9036_wake_lock, HZ * 1);
            if (val) {
                ...
                if (1 == index) {
                    hx9036_pdata.chs_info[0].enabled = true;
                    hx9036_pdata.chs_info[3].enabled = true;
                    hx9036_pdata.chs_info[2].enabled = true;
                    hx9036_pdata.grip_status = SAR_STATE_FAR;
                    PRINT_DBG("hx9036_pdata.grip_status  = %d \n", hx9036_pdata.grip_status);
                } else if (5 == index) {
                    hx9036_pdata.chs_info[4].enabled = true;
                } else if (7 == index) {
                    hx9036_pdata.chs_info[6].enabled = true;
                }
                ...
                if (1 == index) {
                    hx9036_pdata.chs_info[0].enabled = false;
                    hx9036_pdata.chs_info[3].enabled = false;
                    hx9036_pdata.chs_info[2].enabled = false;
                    hx9036_pdata.grip_status = SAR_STATE_FAR;
                    PRINT_DBG("hx9036_pdata.grip_status  = %d \n", hx9036_pdata.grip_status);
                } else if (5 == index) {
                    hx9036_pdata.chs_info[4].enabled = false;
                } else if (7 == index) {
                    hx9036_pdata.chs_info[6].enabled = false;
                }
                //input_report_rel(hx9036_pdata.chs_info[index].hx9036_input_dev, REL_MISC, SAR_STATE_FAR);
                //input_report_rel(hx9036_pdata.chs_info[index].hx9036_input_dev, REL_X, 2);
                //input_sync(hx9036_pdata.chs_info[index].hx9036_input_dev);
...
// - wt add Adaptive ss sensor hal-vts fail,ludandan2 20230815, end
        hx9036_pdata.chs_info[ii].hx9036_sensors_classdev.sensor_name = hx9036_pdata.chs_info[ii].hx9036_input_dev->name;
        hx9036_pdata.chs_info[ii].hx9036_sensors_classdev.vendor = "TYHX";
                                      ...
                                      char *buf)
{
    ...
    len += snprintf(buf+len, PAGE_SIZE-len,
                    "CH%d_background_cap=%d;", 0, data_offset_dac[1] * 26 / 1000);
    len += snprintf(buf+len, PAGE_SIZE-len,
                    "CH%d_refer_channel_cap=%d;", 0, data_offset_dac[0] * 26 / 1000);//cankaotongdao
    hx9036_sample();
                                      ...
                                      char *buf)
{
    ...
    hx9036_read_offset_dac();
    len += snprintf(buf+len, PAGE_SIZE-len,
                    "CH%d_background_cap=%d;", 1, data_offset_dac[3] * 26 / 1000);
    len += snprintf(buf+len, PAGE_SIZE-len,
                    "CH%d_refer_channel_cap=%d;", 1, data_offset_dac[2] * 26 / 1000);//cankaotongdao
    hx9036_sample();
    len += snprintf(buf+len, PAGE_SIZE-len, "CH%d_diff=%d\n", 1, data_diff[3]);
    return len;
}
                                      ...
                                      char *buf)
{
    ...
    hx9036_read_offset_dac();
    len += snprintf(buf+len, PAGE_SIZE-len,
                    "CH%d_background_cap=%d;", 2, data_offset_dac[5] * 26 / 1000);
    len += snprintf(buf+len, PAGE_SIZE-len,
                    "CH%d_refer_channel_cap=%d;", 2, data_offset_dac[4] * 26 / 1000);//cankaotongdao
    hx9036_sample();
    len += snprintf(buf+len, PAGE_SIZE-len, "CH%d_diff=%d\n", 2, data_diff[5]);
    return len;
}
                                      ...
                                      char *buf)
{
    ...
    hx9036_read_offset_dac();
    len += snprintf(buf+len, PAGE_SIZE-len,
                    "CH%d_background_cap=%d;", 3, data_offset_dac[7] * 26 / 1000);
    len += snprintf(buf+len, PAGE_SIZE-len,
                    "CH%d_refer_channel_cap=%d;", 3, data_offset_dac[6] * 26 / 1000);//cankaotongdao
    hx9036_sample();
    len += snprintf(buf+len, PAGE_SIZE-len, "CH%d_diff=%d\n", 3, data_diff[7]);
    return len;
}
...
void exit_anfr_hx9036(void)
{
    ...
    PRINT_INF("exit_anfr_hx9036!!!\n");
}
EXPORT_SYMBOL(exit_anfr_hx9036);
#endif
...
static struct class_attribute class_attr_sar_cali =  __ATTR(sar_cali, 0664, NULL, hx9036_class_sar_cali);
