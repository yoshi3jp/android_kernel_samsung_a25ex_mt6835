...
void nvt_irq_enable(bool enable)
{
	...
	if (ts->client->irq)
		...
		input_err(true, &ts->client->dev, "%s: irq is not registered\n", __func__);
		return;
	}
	...
	input_info(true, &ts->client->dev, "%s: enable=%d, desc->depth=%d\n", __func__, enable, desc->depth);
}
...
void nvt_irq_disable_nosync(void)
{
	...
	input_info(true, &ts->client->dev, "%s: desc->depth=%d\n", __func__, desc->depth);
}
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		...
		if ((ts->power_status == LP_MODE_STATUS) &&
			(ts->lowpower_mode || ts->lcdoff_test) &&
			!(ts->prox_power_off || ts->ear_detect_mode))
#else
		if ((ts->power_status == LP_MODE_STATUS) &&
			(ts->lowpower_mode || ts->lcdoff_test))
#endif
...
************************************************************/
/*void location_detect(struct nvt_ts_data *ts, char *loc, int x, int y)
{
	int i;
...
static irqreturn_t nvt_ts_work_func(int irq, void *data)
{
	...
	if (ts->power_status == LP_MODE_STATUS) {
		__pm_wakeup_event(ts->sec_ws, 500);
...
*/
	...
	if (nvt_wdt_fw_recovery(point_data)) {
		__pm_wakeup_event(ts->sec_ws, 1000);
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	ts->client->cs_setup.value = 1;
	ts->client->cs_setup.unit = 0;
	ts->client->cs_hold.value = 1;
	ts->client->cs_hold.unit = 0;
	ts->client->cs_inactive.value = 1;
	ts->client->cs_inactive.unit = 0;
	...
	ts->input_dev_proximity = devm_input_allocate_device(&client->dev);
	...
	ts->sec_ws = wakeup_source_register(NULL, "TSP");
...
err_init_sec_fn:
	wakeup_source_unregister(ts->sec_ws);
#if PROXIMITY_FUNCTION
