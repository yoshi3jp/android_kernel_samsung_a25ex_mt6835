...
static void nvt_ts_calc_buff(struct nvt_ts_data *ts, int *buff, int *min, int *max)
{
	...
	for (i = 0; i < ts->platdata->y_num; i++) {
		for (j = 0; j < ts->platdata->x_num; j++) {
			*min = MIN(*min, buff[i * ts->platdata->x_num + j]);
			*max = MAX(*max, buff[i * ts->platdata->x_num + j]);
		}
	}
}
...
//	struct nvt_ts_data *ts = container_of(sec, struct nvt_ts_data, sec);
	...
	} else if (ts->fw_ver_ic[0] == 0x3B) {
		snprintf(buff, sizeof(buff), "%s", "NT36528A");
...
static void set_sip_mode(void *device_data)
{
	...
	if (ts->power_status != POWER_ON_STATUS) {
		input_err(true, &ts->client->dev, "%s: SKIP, POWER_STATUS : %d!\n", __func__, ts->power_status);
		goto out;
	}
					...
					char *buf)
{
	...
	if (ts->power_status == POWER_ON_STATUS || ts->power_status == LP_MODE_EXIT)
		enabled = true;

	input_info(true, &ts->client->dev, "%s: power_status %d, enabled %d\n", __func__, ts->power_status, enabled);

	return snprintf(buf, SEC_CMD_BUF_SIZE, "%d", enabled);
}
...
int nvt_ts_sec_fn_init(struct nvt_ts_data *ts)
{
	...
	ret = sec_cmd_init_without_platdata(&ts->sec, sec_cmds, ARRAY_SIZE(sec_cmds), SEC_CLASS_DEVT_TSP, &cmd_attr_group);
	if (ret < 0) {
