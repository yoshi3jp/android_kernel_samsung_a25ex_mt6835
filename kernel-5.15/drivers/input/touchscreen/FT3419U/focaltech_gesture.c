...
// +S98901AA1 zhangjian.wt,add,20240612,add double_click
...
// +S98901AA1 zhangjian.wt,add,20240612,add double_click
bool fts_double_gesture_status;
// -S98901AA1 zhangjian.wt,add,20240612,add double_click

// +S98901AA1 zhangjian.wt,add,20240619,add SwipeUp
bool fts_swipe_up_gesture_status;
// -S98901AA1 zhangjian.wt,add,20240619,add SwipeUp
...
#ifdef SEC_TSP_FACTORY_TEST
extern int sec_ex_mode_switch(u8 mode, u8 value);
#endif
// -S98901AA1 zhangjian.wt,add,20240612,add touch sensitivity func
...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    case GESTURE_RIGHT:
        ...
        break;
    case GESTURE_UP:
// +S98901AA1 zhangjian.wt,add,20240612,add SwipeUp
        gesture = KEY_BLACK_UI_GESTURE;
        FTS_DEBUG("Gesture Code = 0x%x", gesture);
// -S98901AA1 zhangjian.wt,add,20240612,add SwipeUp
        break;
    case GESTURE_DOWN:
...
// +S98901AA1 zhangjian.wt,add,20240612,add double_click
        gesture = GESTURE_DOUBLE_TAP;// KEY_GESTURE_U
// -S98901AA1 zhangjian.wt,add,20240612,add double_click
        break;
    case GESTURE_O:
...
static void aot_enable(void *device_data)
{
    ...
    if(buf[0] == 1){
        fts_double_gesture_status = 1;
    }else{
        fts_double_gesture_status = 0;
        if (ts_data->suspended) {
                FTS_INFO("Turn off aot_gesture in off screen state.\n");
                ts_data->suspended_gesture_support = true;
        }
    }
    if ( fts_double_gesture_status || fts_swipe_up_gesture_status) {
    ...
    FTS_INFO("fts_data->gesture_support == %d\n", fts_data->gesture_support);
...
static void spay_enable(void *device_data)
{
    ...
    FTS_INFO("spay_enable Enter.\n");
    sec_cmd_set_default_result(sec);

    buf[0] = sec->cmd_param[0];

    ts_data->spay_enable = buf[0];
    if (ts_data->spay_enable)
    ...
    if(buf[0] == 1){
        fts_swipe_up_gesture_status = 1;
    }else{
        fts_swipe_up_gesture_status = 0;
        if (ts_data->suspended) {
            FTS_INFO("Turn off spay_gesture in off screen state.\n");
            ts_data->suspended_gesture_support = true;
        }
    }
    if ( fts_double_gesture_status || fts_swipe_up_gesture_status ) {
...
static void glove_mode(void *device_data)
{
    ...
    if(buf[0] == 1){
        if (!fts_data->glove_mode) {
            ...
            ret = sec_ex_mode_switch(0, ENABLE);
            if (ret >= 0) {
        ...
        if (fts_data->glove_mode) {
            ...
            ret = sec_ex_mode_switch(0, DISABLE);
            if (ret >= 0) {
...
// -S98901AA1 zhangjian.wt,add,20240612,add double_click
// +S98901AA1 zhangjian.wt,add,20240612,add SwipeUp
    input_set_capability(input_dev, EV_KEY, KEY_BLACK_UI_GESTURE);
// -S98901AA1 zhangjian.wt,add,20240612,add SwipeUp
...
// -S98901AA1 zhangjian.wt,add,20240612,add double_click
// +S98901AA1 zhangjian.wt,add,20240612,add SwipeUp
    __set_bit(KEY_BLACK_UI_GESTURE, input_dev->keybit);
// -S98901AA1 zhangjian.wt,add,20240612,add SwipeUp
...
// +S98901AA1 zhangjian.wt,add,20240612,add sec_tsp node
#ifdef SEC_TSP_FACTORY_TEST
    ret = sec_cmd_init(&fts_data->sec, fts_commands,
				ARRAY_SIZE(fts_commands), SEC_CLASS_DEVT_TSP);
    if (ret < 0) {
