...
/* define level */
#define TORCH_CURRENT_MIN 50  // 50mA
#define TORCH_CURRENT_MAX 275 //275mA
...
int32_t aw36518_fled_mode_ctrl(int state, uint32_t brightness)
{
	...
	/* brightness to irq */
	iq_cur = brightness;
	if (state == AW36518_FLED_MODE_TORCH_FLASH || state == AW36518_FLED_MODE_PRE_FLASH) {
		if (brightness < TORCH_CURRENT_MIN)
			iq_cur = TORCH_CURRENT_MIN;
		else if (brightness > TORCH_CURRENT_MAX)
			iq_cur = TORCH_CURRENT_MAX;
	}
	...
	case AW36518_FLED_MODE_TORCH_FLASH: /* TORCH FLASH */
		aw36518_set_torch_cur(pdata, iq_cur);
