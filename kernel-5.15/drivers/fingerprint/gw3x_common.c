...
ioctl_failed:
	mutex_unlock(&gf_dev->ioctl_mutex);
	return retval;
}
...
#ifndef ENABLE_SENSORS_FPRINT_SECURE
	if (!of_property_read_u32(np, "goodix,spi_cs_setup_ns_delay", &cs_setup_time)) {
#if KERNEL_VERSION(5, 15, 0) <= LINUX_VERSION_CODE
		if (cs_setup_time != 0) {
			gf_dev->spi->cs_setup.value = cs_setup_time;
			gf_dev->spi->cs_setup.unit = SPI_DELAY_UNIT_NSECS;
		}
#endif
		pr_info("spi_cs_setup_ns_delay: %d ns\n", gf_dev->spi->cs_setup.value);
	} else
		pr_info("not use spi_cs_setup_ns_delay\n");
#endif
...
gw3x_probe_get_gpio:
	mutex_destroy(&gf_dev->ioctl_mutex);
