...
static int force_scheduler_to_exit_sleep(struct kbase_device *kbdev)
{
	...
	if (ret) {
		...
		mtk_logbuffer_print(&kbdev->logbuf_exception,
			"[%llxt] Wait for MCU wake up failed on forced scheduler suspend\n",
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_print(&kbdev->logbuf_exception,
			"[%llxt] Wait for pm state change failed on forced scheduler suspend\n",
			mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception));
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
		goto out;
	}
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
			mtk_logbuffer_print(&kbdev->logbuf_exception,
				"[%llxt] Timeout (%d ms) waiting for queue to start on csi %d bound to group %d on slot %d\n",
				mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception), kbdev->csf.fw_timeout_ms,
				csi_index, group->handle, group->csg_nr);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
			if (kbase_prepare_to_reset_gpu(kbdev, RESET_FLAGS_NONE))
				kbase_reset_gpu(kbdev);
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_print(&kbdev->logbuf_exception,
			"[%llxt] Timeout (%d ms) waiting for queue to stop on csi %d bound to group %d on slot %d\n",
			mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception), kbdev->csf.fw_timeout_ms,
			queue->csi_index, group->handle, group->csg_nr);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
					mtk_logbuffer_print(&kbdev->logbuf_exception,
						"[%llxt] Timeout (%d ms) waiting for queue stop ack on csi %d bound to group %d on slot %d\n",
						mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception), kbdev->csf.fw_timeout_ms,
						queue->csi_index,
						group->handle, group->csg_nr);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_print(&kbdev->logbuf_exception,
			"[%llxt] Group-%d failed to get a slot for stopping the queue on csi %d (timeout %d ms)\n",
			mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception),
			group->handle, queue->csi_index,
			group_schedule_timeout);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_print(&kbdev->logbuf_exception,
			"[%llxt] Wait for MCU active failed for stopping queue on csi %d bound to group %d of context %d_%d on slot %d\n",
			mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception),
			queue->csi_index, queue->group->handle,
			queue->kctx->tgid, queue->kctx->id,
			queue->group->csg_nr);
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
			mtk_logbuffer_print(&kbdev->logbuf_exception,
				"[%llxt] slot %d timeout (%d ms) on up-running\n",
				mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception),
				slot, kbdev->csf.fw_timeout_ms);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
static void detach_from_sched_reclaim_mgr(struct kbase_context *kctx)
{
	...
	if (!list_empty(&heap_info->mgr_link)) {
		...
		dev_vdbg(kctx->kbdev->dev, "Reclaim_mgr_detach: ctx_%d_%d, flags = 0x%x\n",
...
static void attach_to_sched_reclaim_mgr(struct kbase_context *kctx)
{
	...
	dev_vdbg(kctx->kbdev->dev, "Reclaim_mgr_attach: ctx_%d_%d, est_count_pages = %u\n",
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_print(&kbdev->logbuf_regular,
			"term request timeout (%d ms) for group %d of context %d_%d on slot %d, try again\n",
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
			mtk_logbuffer_print(&kbdev->logbuf_exception,
				"[%llxt] term request timeout (%d ms) for group %d of context %d_%d on slot %d\n",
				mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception), kbdev->csf.fw_timeout_ms,
				group->handle, group->kctx->tgid,
				group->kctx->id, group->csg_nr);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
				mtk_logbuffer_print(&kbdev->logbuf_exception,
					"[%llxt] Group %d of context %d_%d on slot %u failed to suspend (timeout %d ms)\n",
					mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception),
					group->handle, group->kctx->tgid,
					group->kctx->id, i,
					kbdev->csf.fw_timeout_ms);
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
			mtk_logbuffer_print(&kbdev->logbuf_regular,
				"Timeout (%d ms) waiting for CSG slots to start, slots: 0x%*pb, try again\n",
				(int)kbdev->csf.fw_timeout_ms / 10,
				num_groups, slot_mask);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
			mtk_logbuffer_print(&kbdev->logbuf_exception,
				"[%llxt] Timeout (%d ms) waiting for CSG slots to start, slots: 0x%*pb\n",
				mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception),
				kbdev->csf.fw_timeout_ms,
				num_groups, slot_mask);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_print(&kbdev->logbuf_exception,
			"[%llxt] Timeout (%d ms) on CSG_REQ:EP_CFG, skipping the update wait: slot mask=0x%lx\n",
			mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception),
			kbdev->csf.fw_timeout_ms,
			slot_mask[0]);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
		...
		struct kbase_context *kctx, struct list_head *evicted_groups)
{
	...
	for (slot = 0; slot < num_groups; slot++) {
		...
		if (group && group->kctx == kctx) {
			...
			dev_info(kbdev->dev, "Evicting group [%d] running on slot [%d] due to reset",
				group->handle, group->csg_nr);
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
	mtk_logbuffer_type_print(kbdev, MTK_LOGBUFFER_TYPE_ALL,
			"Evicting context %d_%d slots: 0x%lx\n",
...
static void scheduler_check_pmode_progress(struct kbase_device *kbdev)
{
	...
	dev_vdbg(kbdev->dev, "Protected mode progress timeout: %llu >= %llu",
...
static void scheduler_ctx_scan_groups(struct kbase_device *kbdev,
		struct kbase_context *kctx, int priority, struct list_head *privileged_groups,
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_print(&kbdev->logbuf_exception,
			"[%llxt] Wait for MCU power on failed on scheduling tick/tock\n",
			mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception));
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
			mtk_logbuffer_print(&kbdev->logbuf_regular,
				"Timeout waiting for CSG slots to suspend, slot_mask: 0x%*pb, try again\n",
				num_groups, slot_mask_local);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
			mtk_logbuffer_print(&kbdev->logbuf_exception,
				"[%llxt] Timeout waiting for CSG slots to suspend, slot_mask: 0x%*pb\n",
				mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception),
				num_groups, slot_mask_local);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
		mtk_logbuffer_print(&kbdev->logbuf_exception,
			"[%llxt] Timeout waiting for cache clean to complete before reset\n",
			mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception));
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
		if (!ret)
			ret = ret2;
...
#if IS_ENABLED(CONFIG_MALI_MTK_LOG_BUFFER)
			mtk_logbuffer_print(&kbdev->logbuf_exception,
				"[%llxt] Timeout waiting for the group %d to suspend on slot %d\n",
				mtk_logbuffer_get_timestamp(kbdev, &kbdev->logbuf_exception),
				group->handle, group->csg_nr);
#endif /* CONFIG_MALI_MTK_LOG_BUFFER */
			goto exit;
...
int kbase_csf_scheduler_handle_runtime_suspend(struct kbase_device *kbdev)
{
	...
	dev_vdbg(kbdev->dev, "Handling runtime suspend");
	...
	if (ret) {
		dev_vdbg(kbdev->dev, "Aborting runtime suspend (grps: %d)",
			 atomic_read(&scheduler->non_idle_offslot_grps));
				    ...
				    struct kbase_csf_tiler_heap_shrink_control *shrink_ctrl)
{
	...
	list_for_each_entry_safe(info, tmp, &mgr->candidate_ctxs, mgr_link) {
		...
		dev_vdbg(kctx->kbdev->dev, "kctx_%d_%d heap pages count : %u\n", kctx->tgid,
				...
				struct kbase_csf_tiler_heap_shrink_control *shrink_ctrl)
{
	...
	dev_vdbg(kbdev->dev, "Reclaim CSF heap free heap pages: %lu (processed kctxs: %u)\n", freed,
		cnt_ctxs);
					  ...
					  struct kbase_csf_tiler_heap_shrink_control *shrink_ctrl)
{
	...
	dev_vdbg(kbdev->dev, "Reclaim count unused pages: %lu (scan: %lu, extra_est: %lu, %d/)\n",
